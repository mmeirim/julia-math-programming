...
function criaGAP(data::AssignmentProblem)
    ...
    global model = BlockModel(coluna)
...
function my_pricing_callback(cbdata)
    ...
    optimal, jobs_assigned_to_cur_machine = solveKnapsack(Knapsack(data.capacities[cur_machine], data.consumptions[cur_machine, :],profits), :Heuristic)
    if optimal == 0
        optimal, jobs_assigned_to_cur_machine = solveKnapsack(Knapsack(data.capacities[cur_machine], data.consumptions[cur_machine, :],profits), :ExpandingCore)
    end

    # optimal, jobs_assigned_to_cur_machine = solve_knapsack(red_costs, data.consumptions[cur_machine, :],data.capacities[cur_machine])

    if optimal == 0
        jobs_assigned_to_cur_machine = []
    end
